...
- [ ] 7.2 **[long-run]** On the next `pnr.yml` run, confirm the same for its `checkout`, two `upload-artifact`, and four `azure/login` steps — the `azure/login` sites gate VM start/deallocate and checkpoint upload, so a failure there risks a VM cost leak (see the existing deallocation requirement). Verify: start and stop jobs both succeed, checkpoints upload, VM ends deallocated. — Not yet run since the merge (last run 2026-09-16, before #33 landed 2026-09-17). Weekly schedule is Fridays 18:00 UTC; dispatchable sooner if wanted.
