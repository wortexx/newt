...
- **[No Azure subscription appropriate for this yet]** → **current blocker.** The only `az` login available at implementation time was the user's employer tenant; provisioning was deliberately not attempted there (D1). Tasks 1.1a–1.1d stay unstarted until the user supplies a personal-appropriate subscription. This gates the entire workflow-file task (3.1) and everything after it.
