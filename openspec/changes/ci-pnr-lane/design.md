...
- **[`grt` itself fails under the new OpenROAD (gate unreachable), not just `drt`]** → same mitigation as above; additionally the gate is driver configuration, so a temporarily narrower gate (through `cts`) can ship first with the spec-gap explicitly tracked, rather than shipping nothing. Spec change would be required — do not do this silently. **What actually happened (task 2.4)**: `grt` didn't fail outright, it just needed far more wall-clock time than any timeout given it — `global_route -congestion_iterations 80` (unmodified from `chip.tcl`, which had no external timeout to race against) measured at ~6h51m of initial routing plus ~15min per congestion iteration, i.e. a ~27h+ total run at 80 iterations. Resolved without narrowing the gate: cut to `-congestion_iterations 20` (~11–13h, `-allow_congestion` already means it can stop early and accept an imperfect result) with `PNR_TIMEOUT_GRT` raised to match (16h). No spec change needed.
- **[Manual out-of-band work on the shared runner workspace has no collision protection from real scheduled workflows]** → not anticipated by the original design (the "contend for the one runner" risk below only covers two *real* Actions jobs, which the single-runner queue serializes for free). Hit twice for real during task 2.4 (2026-09-04 ~07:20 UTC, 2026-09-05 ~07:02 UTC): driving the flow by hand via `docker exec` on the runner's own `_work` directory never registers as a job with the Actions runner, so `synth.yml`'s nightly schedule fires right through it and its `actions/checkout` wipes the shared workspace mid-run (the second time, switching the workspace's git HEAD to an unrelated `main` commit) — destroying hours of checkpointed progress both times, with no remote backup to recover from (task 1.3's checkpoint upload only runs inside the real `pnr.yml` workflow, never during manual bring-up). Mitigation going forward: do real bring-up through the actual `pnr.yml` workflow (protected by the single-runner queue) rather than ad-hoc `docker exec`; task 3.4 proposed a host-level lock file as defense-in-depth for any future manual intervention. **Dropped 2026-09-13**: the manual practice ended, and `PNR_RESUME_EXCLUDE`/`PNR_STOP_AFTER` plus checkpoint restore made targeted single-stage testing possible through the workflow itself (task 2.5(b) ran that way in 16 minutes), removing the reason to hand-drive the runner. Residual risk accepted: a future hand-driven session would again be unprotected.
...
- ~~Task 3.4's lock-file staleness policy~~ — **moot**: task 3.4 was dropped (see the Risks entry above). Original question: a lock left behind by a crashed session or VM reboot must not deadlock every future run forever. Options not yet decided between: a max-age fallback (treat a lock older than N hours as stale and proceed with a warning), or requiring explicit manual clearing (recorded in the runbook, matching how this project already handles stale `git` `index.lock` files today) — the latter is simpler but means a crash requires a human to unblock the next scheduled run.
