...
- [ ] 4.2 **[docker]** Open the PR and confirm the PR run builds all stages and the smoke test passes without publishing (spec "Pull request build-verifies only" scenario). If a cold-cache build exceeds runner limits, split stages into matrix jobs per design D5 fallback. — PR opened: https://github.com/wortexx/newt/pull/2, workflow run in progress.
