...
- `verible-verilog-lint` is not invoked anywhere in this repo today — this change is what wires it in for the first time. `iguana.mk` has no "lint changed RTL only" target; `verilator --lint-only` today only runs implicitly as part of `ig-verilator-model` over the whole file list (`target/verilator/verilator.mk`), not scoped to a diff.
- `bender check` does not exist - confirmed absent from both the image's pinned bender 0.27.4 and a newer 0.32.0's command list. It was a stale, never-verified claim already sitting in `docs/infra-plan.md`'s Phase 3 table before this change (found via a real CI failure - `error: Unknown command 'check'.` - not caught by local testing beforehand, since that testing used the host's own newer bender instead of the pinned container one). `bender sources` is the real equivalent used instead: it fully resolves `Bender.yml`/`Bender.lock` and errors on a genuine inconsistency, with no `check`-specific behavior to lose.
...
`git diff --name-only origin/main...HEAD -- '*.sv' '*.svh'` (PR) or the push's before/after SHA range, feeding both `verible-verilog-lint` and a per-file `verilator --lint-only` (each file compiled standalone against the project's include paths, extracted from `bender script verilator`'s own `+incdir+`/`+define+` output rather than hand-maintaining a second copy - since a full-elaboration `--lint-only` run needs the complete Bender file list and is already covered at build time by `ig-verilator-model`/`ig-verilator-flist`, not something worth re-running as a per-PR lint step). `bender sources` runs unscoped (it validates `Bender.yml`/`Bender.lock` consistency, not per-file).
